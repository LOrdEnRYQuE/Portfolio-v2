...
interface ProjectDetailProps {
  project: Project & { _id?: string; _creationTime?: number };
}
    ...
    <article className="py-24 px-6 md:px-10 max-w-4xl mx-auto min-h-screen">
      ...
      >
        ...
        <div className="absolute inset-0 bg-linear-to-br from-background via-accent/5 to-accent/10 flex items-center justify-center p-12 md:p-24">
          <motion.div 
            animate={{ 
              boxShadow: ["0 0 40px rgba(255,255,255,0.05)", "0 0 80px rgba(255,255,255,0.15)", "0 0 40px rgba(255,255,255,0.05)"],
      ...
      <div className="space-y-20">
        {(project.brief || !t(`projects.${project.slug}.brief`).includes('projects.')) && (
          <motion.section 
        ...
        {(project.challenge || !t(`projects.${project.slug}.challenge`).includes('projects.')) && (
          <motion.section 
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
        ...
        {(project.solution || !t(`projects.${project.slug}.solution`).includes('projects.')) && (
          <motion.section 
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
        ...
        >
          ...
          <div className="flex flex-wrap gap-3">
            {Array.isArray(project.stack) ? project.stack.map((tech: string) => (
              <Badge key={tech} variant="accent" className="px-4 py-1.5 text-sm">
                {tech}
              </Badge>
            )) : JSON.parse(project.stack || "[]").map((tech: string) => (
              <Badge key={tech} variant="accent" className="px-4 py-1.5 text-sm">
                {tech}
              </Badge>
            ))}
